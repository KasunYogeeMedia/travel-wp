...
class Cart
{
    ...
    function getItems()
    {
        ...
        if(is_array($this->items) && count($this->items) > 0) return $this->items;
	...
	function isRecurring()
	{
		...
		if($dynamic_rec === 'NO') return false;
		if($dynamic_rec === 'YES') return true;
    ...
    function applyCoupon($code)
    {
		...
		if($discount > 0) {
			$coupon_info = array('code' => $code, 'coupon' => $code, 'discount' => $discount, 'coupon_discount' => $discount, 'product_id' => $coupon->product, 'note' => wpdm_sanitize_var($coupon->description, "kses"));
