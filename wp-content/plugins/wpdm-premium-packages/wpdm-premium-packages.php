...
if (!class_exists('WPDMPremiumPackage')):
	...
	class WPDMPremiumPackage
	{
		...
		private function init_hooks()
		{
		    ...
		    register_activation_hook( __FILE__, [ '\WPDMPP\Libs\Installer', 'init' ] );
		...
		function calculate_tax(){
            ...
            $updates = [ 'tax' => wpdmpp_price_format($tax_total), 'total' => wpdmpp_price_format($total_including_tax) , 'subtotal' => $cartsubtotal, 'dis' => $cartdiscount, 'order' => Session::get('orderid'), 'payment_button' => $customPayButton ];
		...
		function settings_tab($tabs){
			$tabs['ppsettings'] = wpdm_create_settings_tab('ppsettings', 'Premium Package', array( $this, 'settings' ), $icon = 'fa-solid fa-basket-shopping');
			return $tabs;
		}
		...
		function download()
		{
		    ...
		    if(wpdm_query_var('wpdmppd') !== '' || wpdm_query_var('wpdmppdl') !== '') {
		        ...
		        if(!is_array($wpdmdd) || !isset($wpdmdd['ID'], $wpdmdd['oid'])) Messages::error(__( "&mdash; Invalid download link &mdash;", "wpdm-premium-packages" ), 1);
                ...
                $package['files'] = WPDM()->package->getFiles($PID, true); //maybe_unserialize(get_post_meta($PID, '__wpdm_files', true));
                ...
                if (@in_array($PID, $items) && $OID != '' && is_user_logged_in() && $current_user->ID == $odata->uid && $odata->order_status == 'Completed') {
                    ...
                    if (count($cfiles) > 0 && !isset($cfiles[wpdm_query_var('ind')])) {
                        if (count($cfiles) > 1) {
                            ...
                            if(!file_exists($file)) $file = WPDM()->fileSystem->locateFile($file);
